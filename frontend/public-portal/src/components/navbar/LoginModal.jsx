...
export default function LoginModal({ isOpen, onClose, switchToRegister }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { role, fullName, profileImageUrl } = res.data;
      
      localStorage.setItem('user', JSON.stringify({ email, fullName, role, profileImageUrl }));
      ...
      <ModalContent className="dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 max-w-sm rounded-lg">
        <ModalBody className="p-6">
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold tracking-tight text-zinc-950 dark:text-zinc-50">Sign In</h2>
          ...
          <p className="text-center text-xs text-zinc-500 dark:text-zinc-400 mt-4">
            ...
            <button onClick={switchToRegister} className="text-zinc-900 dark:text-zinc-50 hover:underline font-semibold">
              Create Account
            </button>
          </p>
        </ModalBody>
